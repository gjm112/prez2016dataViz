...
library(mapproj)
library(dplyr)
png("/Users/gregorymatthews/Dropbox/StatsInTheWild/greenParty.png",res=300,units="in",w=20,h=10)
...
prez<-prez[prez$cand=="Jill Stein",]
...
col <- unlist(lapply(map("county", plot=FALSE)$names,function(x){
  ...
  temp$pct[temp$pct>0.1]<-0.1
  if (dim(temp)[1]>0){
  out<-NA
  if (!is.na(temp$pct)){
    out<-rgb(1-temp$pct/.1,1,1-temp$pct/.1,1)  
  }
...
title("Election 2016: Green Party")


colleg<-rgb(1-seq(0,0.1,length=5)/0.1,1,1-seq(0,0.1,length=5)/0.1,1) 
leg.txt<-paste0(c(round(100*seq(0,0.1,length=5),2)),"%")
