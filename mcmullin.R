...
library(mapproj)
library(dplyr)
png("/Users/gregorymatthews/Dropbox/StatsInTheWild/McMullin.png",res=300,units="in",w=20,h=10)
...
prez<-prez[prez$cand=="Evan McMullin",]
...
col<-rep("white",length(map("county", plot=FALSE)$names))

prez$pct<-prez$votes/prez$total_votes
#prez<-prez[!is.na(prez$fips),]

col <- unlist(lapply(map("county", plot=FALSE)$names,function(x){
  temp<-prez[as.character(prez$polyname)==x,]
  temp$pct[temp$pct>0.3]<-0.3
  out<-rgb(1,1,1,1)  
  if (dim(temp)[1]>0){
    if (!is.na(temp$pct)){
      out<-rgb(1-temp$pct/0.3,1-temp$pct/0.3,1-temp$pct/0.3,1)  
...
title("Election 2016: Evan McMullin")


colleg<-rgb(1-seq(0,0.3,length=5)/0.3,1-seq(0,0.3,length=5)/0.3,1-seq(0,0.3,length=5)/0.3,1) 
leg.txt<-paste0(c(round(100*seq(0,0.3,length=5),2)),"%")
