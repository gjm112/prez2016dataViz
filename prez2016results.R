...
library(mapproj)
library(dplyr)
png("/Users/gregorymatthews/Dropbox/StatsInTheWild/candidatesTWO_dens1.png",res=300,units="in",w=20,h=10)
...
prez$total_votes2<-rep(tots[,2],each=2)
prez<-prez[prez$cand=="Donald Trump",]
prez$pct<-prez$votes/prez$total_votes2
#names(prez)<-c("fips","votes")
...
col <- unlist(lapply(map("county", plot=FALSE)$names,function(x){
  ...
  temp$total_votes2[temp$total_votes2>50000]<-50000
  out<-rgb(temp$pct,0,1-temp$pct,1)  
  return(out)
}))
...
colleg<-rgb(seq(0,1,length=9),0,1-seq(0,1,length=9),50000/50000) 
leg.txt<-paste0(c(round(100*seq(0,1,length=9),2)),"%")
